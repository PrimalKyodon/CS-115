...
def insertV2(L, i):
    ...
    locVal = search(L, i, L[i])
    val = L[i]
    for j in range(i, locVal, -1):
        L[j] = L[j-1]
    L[locVal] = val
    return L
##################################################
...
def letterCounts(fname):
    ...
    letters = {"non-letter": 0}
    fileInput = open(fname, "r")
    ...
    for j in lines:
        for i in j:
            if letter(i) != "non-letter":
                ...
                letters["non-letter"] += 1
    listLetters = [] 
    for i in letters:
        if i == "non-letter" and letters[i] == 0:
            None
        else:
            listLetters += [(letters[i], i)]
    listLetters.sort()
    listLetters.reverse()
    return listLetters
